import {SonetActorDefs, nuxSchema} from '@sonet/api'
...
export function parseAppNux(nux: SonetActorDefs.Nux): AppNux | undefined {
...
export function serializeAppNux(nux: AppNux): SonetActorDefs.Nux {
  const {data, ...rest} = nux
  const schema = NuxSchemas[nux.id as Nux]

  const result: SonetActorDefs.Nux = {
    ...rest,
    data: undefined,
  }
