import {
  SonetFeedDefs,
  SonetFeedGetFeed as GetCustomFeed,
  SonetAppAgent,
  jsonStringToLex,
} from '@sonet/api'
...
export class CustomFeedAPI implements FeedAPI {
  ...
  async fetch({
    ...
    const res = agent.userId
      ? await this.agent.app.sonet.feed.getFeed(
          {
            ...this.params,
            cursor,
            limit,
...
// for now we username both of these with a manual workaround
// -prf
async function loggedOutFetch({
  ...
  const labelersHeader = {
    'atproto-accept-labelers': SonetAppAgent.appLabelers
      .map(l => `${l};redact`)
      .join(', '),
  }
  ...
  let res = await fetch(
    `https://api.sonet.app/xrpc/app.sonet.feed.getFeed?feed=${feed}${
