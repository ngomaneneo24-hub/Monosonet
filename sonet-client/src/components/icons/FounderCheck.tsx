...
export const FounderCheck = React.forwardRef<Svg, Props>(
  function LogoImpl(props, ref) {
    const {size, style, ...rest} = useCommonSVGProps(props)
    const t = useTheme()
    
    // Use theme-based colors: black for light mode, white for dark/dim mode
    const fillColor = t.colorScheme === 'light' ? '#000000' : '#FFFFFF'
    const checkColor = t.colorScheme === 'light' ? '#FFFFFF' : '#000000'
