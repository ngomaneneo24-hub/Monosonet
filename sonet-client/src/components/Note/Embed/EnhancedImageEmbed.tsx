...
export function EnhancedImageEmbed({
  embed,
  ...rest
}: CommonProps & {
  ...
  if (images.length > 0) {
    ...
    // Single image - use AutoSizedImage
    if (images.length === 1) {
    ...
    // 2 images - use traditional ImageLayoutGrid
    if (images.length === 2) {
      return (
        <View style={[a.mt_sm, rest.style]}>
          <ImageLayoutGrid
    ...
    // 3+ images - use our enhanced carousel
    return (
      <View style={[a.mt_sm, rest.style]}>
        <EnhancedGallery
          images={images}
          ...
          onLongPress={() => {}}
          viewContext={rest.viewContext}
          containerRefs={[]} // We'll need to handle this properly
          thumbDimsRef={{current: []}} // We'll need to handle this properly
        />
      </View>
    )
  }
