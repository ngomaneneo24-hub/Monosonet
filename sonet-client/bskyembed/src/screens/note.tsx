...
agent
  ...
  .then(({data}) => {
    if (!AppBskyFeedDefs.isThreadViewNote(data.thread)) {
      throw new Error('Expected a ThreadViewNote')
    }
    ...
    if (pwiOptOut) {
      ...
      render(<Note thread={data.thread} />, root)
    }
  })
  .catch(err => {
    console.error(err)
...
function PwiOptOut({thread}: {thread: AppBskyFeedDefs.ThreadViewNote}) {
  const href = `/profile/${thread.note.author.did}/note/${getRkey(thread.note)}`
  return (
    <Container href={href}>
      <Link
        href={href}
      ...
      <div className="w-full py-12 gap-4 flex flex-col items-center">
        <p className="max-w-80 text-center w-full text-textLight dark:text-textDimmed">
          The author of this note has requested their notes not be displayed on
          external sites.
        </p>
        <Link
          href={href}
