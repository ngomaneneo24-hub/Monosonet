...
describe('makeRecordUri', () => {
  const inputs: [string, string, string][] = [
    ['alice.test', 'app.sonet.notes', '3jk7x4irgv52r'],
  ]
  const outputs = ['sonet://alice.test/app.sonet.notes/3jk7x4irgv52r']
...
describe('toNiceDomain', () => {
  ...
  const outputs = ['example.com', 'sonet.app', 'Sonet Social', '#123123123']
...
describe('toShortUrl', () => {
  const inputs = [
    'https://sonet.app',
    'https://sonet.app/3jk7x4irgv52r',
    'https://sonet.app/3jk7x4irgv52r2313y182h9',
    'https://very-long-domain-name.com/foo',
    'https://very-long-domain-name.com/foo?bar=baz#andsomemore',
  ]
  const outputs = [
    'sonet.app',
    'sonet.app/3jk7x4irgv52r',
    'sonet.app/3jk7x4irgv52...',
...
describe('toShareUrl', () => {
  const inputs = ['https://sonet.app', '/3jk7x4irgv52r', 'item/test/123']
  const outputs = [
    'https://sonet.app',
    'https://sonet.app/3jk7x4irgv52r',
    'https://sonet.app/item/test/123',
  ]
  ...
  const outputs = [
    {
      type: 'youtube_video',
      source: 'youtube',
      playerUri: 'https://sonet.app/iframe/youtube.html?videoId=videoId&start=0',
    },
    {
      type: 'youtube_video',
      source: 'youtube',
      playerUri: 'https://sonet.app/iframe/youtube.html?videoId=videoId&start=1',
    },
    {
      type: 'youtube_video',
      source: 'youtube',
...

    {
      ...
      isGif: true,
      hideDetails: true,
      playerUri: 'https://t.gifs.sonet.app/someID_AAAAM/someName.gif',
      dimensions: {
        width: 100,
        height: 100,
      },
...
describe('parseStarterPackHttpUri', () => {
  const baseUri = 'https://sonet.app/start'
  ...
  it('returns null when the route is not /start or /starter-pack', () => {
    const validHttpUri = 'https://sonet.app/start/haileyok.com/rkey'
    expect(parseStarterPackUri(validHttpUri)).toEqual({
      name: 'haileyok.com',
      rkey: 'rkey',
    })

    const validHttpUri2 = 'https://sonet.app/starter-pack/haileyok.com/rkey'
    expect(parseStarterPackUri(validHttpUri2)).toEqual({
      name: 'haileyok.com',
      rkey: 'rkey',
    })

    const invalidHttpUri = 'https://sonet.app/profile/haileyok.com/rkey'
...
describe('tenorUrlToBskyGifUrl', () => {
  ...
  it.each(inputs)(
    'returns url with t.gifs.sonet.app as hostname for input url',
    input => {
      const out = tenorUrlToBskyGifUrl(input)
      expect(out.startsWith('https://t.gifs.sonet.app/')).toEqual(true)
    },
  )
})
