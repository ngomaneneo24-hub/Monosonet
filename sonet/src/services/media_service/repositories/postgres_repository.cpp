...
class PostgresRepo final : public MediaRepository {
public:
    explicit PostgresRepo(std::string conn): conn_str_(std::move(conn)) {
...
std::unique_ptr<MediaRepository> CreatePostgresRepo(const std::string& conn_str) {
    try { return std::make_unique<PostgresRepo>(conn_str); } catch (...) { return nullptr; }
}
