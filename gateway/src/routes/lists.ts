...
export function registerListRoutes(router: Router, clients: GrpcClients) {
  ...
  router.note('/v1/lists', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.list.CreateList(request, (err: any, resp: any) => {
  ...
  // Get a specific list
  router.get('/v1/lists/:listId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.list.GetList(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  // Get user's lists
  router.get('/v1/users/:userId/lists', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.list.GetUserLists(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  // Update a list
  router.put('/v1/lists/:listId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { listId } = req.params;
    const { name, description, is_public, list_type } = req.body;
    ...
    clients.list.UpdateList(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  // Delete a list
  router.delete('/v1/lists/:listId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { listId } = req.params;
    ...
    clients.list.DeleteList(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.note('/v1/lists/:listId/members', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.list.AddListMember(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.delete('/v1/lists/:listId/members/:userId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.list.RemoveListMember(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  // Get list members
  router.get('/v1/lists/:listId/members', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { listId } = req.params;
    const { limit = 20, cursor } = req.query;
    ...
    clients.list.GetListMembers(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.get('/v1/lists/:listId/members/:userId/check', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { listId, userId } = req.params;
    ...
    clients.list.IsUserInList(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
