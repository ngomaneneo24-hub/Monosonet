...
export function registerUserRoutes(router: Router, clients: GrpcClients) {
  ...
  router.get('/v1/users/:userId/privacy', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.user.GetUserPrivacy(request, (err: any, resp: any) => {
  ...
  router.put('/v1/users/:userId/privacy', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { userId } = req.params;
    ...
    clients.user.UpdateUserPrivacy(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.get('/v1/users/:userId/profile', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { userId } = req.params;
    ...
    clients.user.GetUserProfile(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
