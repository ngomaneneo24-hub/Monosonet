...
export function registerFeedRoutes(router: Router, clients: GrpcClients) {
  // For You Feed - ML-powered personalized content
  router.get('/v1/feeds/for-you', (req: Request, res: Response) => {
  ...
  router.get('/v1/feeds/video', (req: Request, res: Response) => {
    const limit = Number(req.query.limit || 20);
    const cursor = String(req.query.cursor || '');
    const userId = userIdFromAuth(req);
    ...
    clients.timeline.GetVideoFeed(request, (err: any, resp: any) => {
      if (err) {
  ...
  router.get('/v1/feeds/following', (req: Request, res: Response) => {
    const limit = Number(req.query.limit || 20);
    const cursor = String(req.query.cursor || '');
    const userId = userIdFromAuth(req);
    ...
    clients.timeline.GetFollowingFeed(request, (err: any, resp: any) => {
      if (err) {
  ...
  router.post('/v1/feeds/interactions', (req: Request, res: Response) => {
    ...
    clients.analytics.TrackFeedInteractions(request, (err: any, resp: any) => {
      if (err) {
  ...
  router.get('/v1/feeds/personalization/:userId', (req: Request, res: Response) => {
    ...
    clients.timeline.GetUserPersonalization(request, (err: any, resp: any) => {
      if (err) {
