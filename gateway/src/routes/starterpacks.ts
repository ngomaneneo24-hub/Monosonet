...
export function registerStarterpackRoutes(router: Router, clients: GrpcClients) {
  ...
  router.note('/v1/starterpacks', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.starterpack.CreateStarterpack(request, (err: any, resp: any) => {
  ...
  router.get('/v1/starterpacks/:starterpackId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.starterpack.GetStarterpack(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.get('/v1/users/:userId/starterpacks', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.starterpack.GetUserStarterpacks(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.put('/v1/starterpacks/:starterpackId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { starterpackId } = req.params;
    const { name, description, avatar_url, is_public } = req.body;
    ...
    clients.starterpack.UpdateStarterpack(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.delete('/v1/starterpacks/:starterpackId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { starterpackId } = req.params;
    ...
    clients.starterpack.DeleteStarterpack(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.note('/v1/starterpacks/:starterpackId/items', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.starterpack.AddStarterpackItem(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.delete('/v1/starterpacks/:starterpackId/items/:itemId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.starterpack.RemoveStarterpackItem(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.get('/v1/starterpacks/:starterpackId/items', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { starterpackId } = req.params;
    const { limit = 20, cursor } = req.query;
    ...
    clients.starterpack.GetStarterpackItems(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.get('/v1/starterpacks/suggested', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { limit = 20, cursor } = req.query;
    ...
    clients.starterpack.GetSuggestedStarterpacks(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
