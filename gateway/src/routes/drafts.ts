...
export function registerDraftRoutes(router: Router, clients: GrpcClients) {
  ...
  router.get('/v1/drafts', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.drafts.GetUserDrafts(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  router.get('/v1/drafts/:draftId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    ...
    clients.drafts.GetDraft(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
  ...
  // Update a draft
  router.put('/v1/drafts/:draftId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { draftId } = req.params;
  ...
  // Delete a draft
  router.delete('/v1/drafts/:draftId', verifyJwt, (req: AuthenticatedRequest, res: Response) => {
    const { draftId } = req.params;
    ...
    clients.drafts.DeleteDraft(request, (err: any, resp: any) => {
      if (err) return res.status(400).json({ ok: false, message: err.message });
      if (!resp?.success) {
