...
export function registerMessageWebsocket(server, clients) {
    ...
    wss.on('connection', (ws, req) => {
        ...
        if (maybeChatId && maybeChatId !== 'ws')
            chatFilter = maybeChatId;
        const queryToken = query.get('token');
        if (queryToken)
            userId = getUserIdFromToken(queryToken);
        let grpcStream = null;
        let isOpen = true;
        ...
        ws.on('message', data => {
            ...
            if (!userId)
                return closeWith(1008, 'Unauthorized');
